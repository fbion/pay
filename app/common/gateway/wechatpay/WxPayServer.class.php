...
class Common_util_pub {
    ...
    protected $appId ;
    protected $appSercert ;
    protected $mchId ;
    protected $mchKey ;
    protected $sslFilePath ;

    public function __construct($config) {
        $this->mchId        = $config['partner'] ;
        $this->mchKey       = $config['security_code'] ;
        $this->appId        = $config['appid'] ;
        $this->appSercert   = $config['appsercert'] ;
        $this->sslFilePath  = dirname(__FILE__) . '/cacert/' . $config['partner'] . '.pem' ;
    }
    ...
    public function postXmlSSLCurl($xml,$url,$second=30)
    {
        ...
        ///使用证书：第二种方式，两个文件合成一个.pem文件
        if (file_exists($this->sslFilePath)) {
            //默认格式为PEM，可以注释
            curl_setopt($ch,CURLOPT_SSLCERTTYPE, 'PEM') ;
            curl_setopt($ch,CURLOPT_SSLCERT, $this->sslFilePath) ;
        }
...
class AppApi_pub extends Common_util_pub {
    ...
    public function getParameters() {
        ...
        $appApiObj["partnerId"] = $this->_partnerId ;
        ...
        $appApiObj["signType"] = "MD5" ;
        $appApiObj["sign"] = $this->getSign($appApiObj) ;
        $this->parameters = json_encode($jsApiObj) ;
        return $this->parameters ;
    }
}
